...
impl<'a> Bif<'a> {
    ...
    */
    pub(crate) fn parse_bif_same(&mut self) -> Result<(), BifError> {
        ...
        let param1 = args.get(1).cloned().ok_or_else(|| self.bif_error(BIF_ERROR_ARGUMENTS_NOT_FOUND))?;

        let param2 = args.get(2).cloned().ok_or_else(|| self.bif_error(BIF_ERROR_ARGUMENTS_NOT_FOUND))?;
